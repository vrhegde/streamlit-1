...
"""
    #### The effect of uneven tier distrubution and bonus points on probablity of amdission into exam schools.

    Until 2020, admission to Bostons's three 'exam schools' was determined by using a combination of a standardized test (the ISEE) and the student's grades. Each of these two criteria was weighed equally. Students competed for these seats in a single combined pool. Thus merit, compared on a city-wide basis, was the only metric for admission into these selective schools.

    The new admission policy for these schools has made several changes:
     1. The seats are divided into 8 socio-economic tiers, determined by the census-tract their home is located in. Students can compete only within the assigned tier, and not across different tiers.
     2. ISEE is eliminated.
     3. Grades are weighed 70% and a new test, the NWEA MAP test, is weighted 30%.
     4. The MAP test will not be considered for the 2021-22 admission cycle.

    There are 2 features of this plan, in its current state, that will unfairly disadvantage many students. First, the tiers are divided according to the estimated number of school aged children within a specific age range, and not by the number of eligible applicants (eligible applicants are students with grade average of B or higher). As the number of eligible students is substantially higher in the upper tiers, those students will have a lower probablity of getting selected. Second, every student going to a schools where more than 40% of students are socio-economically challanged will get 10 bonus points (out of a total score of 100) added on to their final score used to determine exam school admissions. These bonus points are determined by the school, and not be the socio-economic situation of the student. As about 80% of BPS students attend bonus-points schools, the remaining 20% of students will be severly disadvantaged.
    ...
    The aim of this project is to demonstrate the effect of bonus points, and of using unevenly distributed tiers, on probablity of admission to selective schools. In this project, we will generate synthetic data (within the bounds of what we know from previous admission cycles) and run repeated simulations to determine the probablity distribution of admissions to these schools.


    #### Method:
    We will first create synthetic data using two different methods. The first without bonus points, and the second with bonus points. We will create two tier distributions: First tiers that have the same number of students (Even distribution), and second with more students in the upper tiers (Skewed distribution). For the skewed tier distribution, relative distribution of students per tier will be estimated by data presented by the school committee and the exam school taks force.

    We will create a form where one can specify the number of applicants, the total number of seats, and the number of trial simulations to run. This will be the input interface for those who want to tinker with the data but are not familier with the workings of a python notebook.

    Synthetic data creation:
    Two functions to create synthetic data will be written. The only difference between the functions is that one does not use bonus points in calculating the final score, and the other does. Two different methods are used to generate data as we want to explore the impact of bonus points on probablity of admission.

    1. A DataFrame containing n (= number of student) rows will be created.
    2. Raw scores will be randomly generated and assigned to each student. Scores will range from 7 to 12 (grades B to A +).
    ...
    4. Weather or not the student gets bonus points will begenerated randomly (yes =1, no =0). Random binary generator will be set such that 80% of the times the number 1 is returned (thus 80% of students get bonus points).
    5. Final scores are calculated by adding 10 points to scaled score if bonus points method used. Alternately, final score will be the same as the scaled score, if non-bonus point method is used.
...
"""
#### What happens when bonus points are added.
"""
